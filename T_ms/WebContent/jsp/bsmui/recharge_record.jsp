...
<html lang="en">
    ...
    function editState(index){
    	 $('#recharge_record').datagrid('selectRow',index);// 关键在这里  
         var row = $('#recharge_record').datagrid('getSelected');
    ...
    function deletedate(){
    	 var _list = {};  
    	 var row = $('#recharge_record').datagrid('getSelections'); //多行选择
    	if(row.length > 0){
    		...
    		$.ajax({  
    		    ...
    		    success: function (data) {  
    		    	if(data[0].msg == "成功"){
						   $('#dlg').dialog('close');
						   alert('删除成功！');
						   $('#recharge_record').datagrid({loadFilter:pagerFilter}).datagrid('loadData', loadData(date,userName,operationid));
					   }
   ...
   function loadData(date,userName,operationid){
		$('#recharge_record').datagrid({
			url: '../../recharge/list.do?date='+date+'&userName='+userName+'&operationid='+operationid,
			title: '用户充值记录',
			height:'560px',
			fitColumns: true,
			columns:[[
				...
				{field:'money',title:'充值余额',width:100},
				{field:'operationid',title:'操作id',width:100},
				{field:'time',title:'充值时间',width:150},
    ...
    <!--客户端分页 -->
    $(function(){$('#recharge_record').datagrid({loadFilter:pagerFilter}).datagrid('loadData', loadData(date,userName,operationid));})
    </script>
    </head>
    <body>
    <table id = "recharge_record"></table>
    <div id="tb" style="padding:5px;height:auto">
		<div>
			充值日期: <input id ="date" name = "date" class="easyui-datebox" style="width:150px">
			用户名:<input id = "userName" class='easyui-textbox' style="width:150px">
			操作id:<input id = "operationid" class='easyui-textbox' style="width:150px">
