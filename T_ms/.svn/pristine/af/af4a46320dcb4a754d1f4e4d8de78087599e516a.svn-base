...
<html lang="en">
    ...
    function formatOper(val,row,index){
    	if(row.state == '已兑换'){
    		...
    		return '<a href="#"  style="text-decoration:none;" onclick="editState('+index+')">兑换</a>';	
    	}
        	...
        	$.ajax({
				   ...
				   success:function(data){
					   if(data[0].msg == "success"){
						   $('#dlg').dialog('close');
						   alert('状态修改成功！');
						   $('#order').datagrid({loadFilter:pagerFilter}).datagrid('loadData', loadData(awardTime,userName,state));
					   }
    ...
    function Search(){
    	awardTime = $("input[name='awardTime']").val();//使用id 获取不了 这里使用name
    	userName = $("#userName").val();
    	state = $('#state').combobox('getValue');<!--easyui中获取select选中值 -->
    //	userid = $("#userid").val();
    	$('#order').datagrid({loadFilter:pagerFilter}).datagrid('loadData', loadData(awardTime,userName,state));
    };
   function loadData(awardTime,userName,state){
		$('#order').datagrid({
			url: '../../award/listBySearch.do?uaerName='+userName+'&awardTime='+awardTime+'&state='+state,
			title: '积分抽奖管理',
			height:'560px',
			fitColumns: true,
			columns:[[
				...
				{field:'awardName',title:'奖品名称',width:150},
				{field:'awardTime',title:'抽奖时间',width:80},
    ...
    <!--客户端分页 -->
    $(function(){$('#order').datagrid({loadFilter:pagerFilter}).datagrid('loadData', loadData(awardTime,userName,state));})
    </script>
    </head>
    <body>
    ...
    <div id="tb" style="padding:5px;height:auto">
		<div>
			抽奖日期: <input id ="awardTime" name = "awardTime" class="easyui-datebox" style="width:150px">
	...
	<div id="dlg" class="easyui-dialog" title="添加食物" style="display: none;overflow: hidden; width:400px;height:400px;padding:10px"
		...
		<form id="addTable" enctype="multipart/form-data" method="post">
			<table>
				<tr><td>添加菜名：</td><td><input id="orderNum" name="orderNum"  class='easyui-textbox' data-options="prompt:'请输入菜名',missingMessage:'此处不能为空',invalidMessage:'长度为3~10之间',required:true,validType:'length[3,10]'" style="width:250px"></td></tr>
