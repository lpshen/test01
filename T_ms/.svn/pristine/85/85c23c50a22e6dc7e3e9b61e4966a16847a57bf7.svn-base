...
<div id="main">
	...
	<!-- 主要部分 -->
				<div>
				<p style="font-size: 30px; " align="center">充值记录</p>
					<table width="90%" align="center" style="border: 2px solid #FFFFFF;">
					<tr><th>用户ID</th><th>充值金额</th><th>时间</th><th>操作ID</th></tr>
					<c:forEach items="${pageModel.list}" var="rechargeRecord">
						<tr ><td align="center">${rechargeRecord.userId}：</td><td align="center">${ rechargeRecord.money}</td><td align="center">${rechargeRecord.time }</td><td align="center">${ rechargeRecord.operationid}</td></tr>
					</c:forEach>
					</table>
					<p align="center">
						<a href="${pageContext.request.contextPath}/recharge/listByPageCZ.do?pageNo=${pageModel.firstPage}&userid=${user.id}">第一页</a>
						<a href="${pageContext.request.contextPath}/recharge/listByPageCZ.do?pageNo=${pageModel.previousPage}&userid=${user.id}">上一页</a>
						<a href="${pageContext.request.contextPath}/recharge/listByPageCZ.do?pageNo=${pageModel.nextPage}&userid=${user.id}">下一页</a>
						<a href="${pageContext.request.contextPath}/recharge/listByPageCZ.do?pageNo=${pageModel.lastPage}&userid=${user.id}">尾页</a>
